...
/*
  ...
  ** JPA를 쓸 때, 가장 걱정하는 점 '데이터 동기화'
     1차 캐시에 데이터가 있고, DB에 반영이 안될 경우 로직에 대해 우려
     결국 이 매커니즘은 트랜잭션 단위로 돌아감
     영속성 컨텍스트와 트랜잭션의 주기를 맞춰서 설계를 해야 문제가 발생하지 않음
     트랜잭션을 시작하고 끝나버리면 영속성 컨텍스트가 거기에 맞춰 시작하고 끝나도록 설계를 해야
     개발할 때, 데이터 동기화 등 문제가 발생하지 않음
